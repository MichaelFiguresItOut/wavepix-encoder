...
export const drawSpiderWebAnimation = (
  ...
  settings.barPlacement.forEach(placement => {
    ...
    if (settings.horizontalOrientation) {
      ...
      // Draw the horizontal oriented web
      drawWebAtPosition(ctx, dataArray, bufferLength, timestamp, settings, 
                        centerX, centerY, canvasWidth, canvasHeight, 
                        currentRainbowSpeed, baseHue);
    }
    ...
    if (settings.verticalOrientation) {
      ...
      // Draw the vertical oriented web
      drawWebAtPosition(ctx, dataArray, bufferLength, timestamp, settings, 
                        centerX, centerY, canvasWidth, canvasHeight, 
                        currentRainbowSpeed, baseHue);
    }
...
// Helper function to draw a spider web at a specific position
function drawWebAtPosition(
  ctx: CanvasRenderingContext2D,
  dataArray: Uint8Array,
  bufferLength: number,
  timestamp: number,
  ...
  const webSize = Math.min(canvasWidth, canvasHeight) * 0.3; // Slightly smaller to fit multiple webs
...
function getFrequencyValue(dataArray: Uint8Array, index: number, totalPoints: number, bufferLength: number): number {
  const pointsPerBin = bufferLength / totalPoints;
  const startIndex = Math.floor(index * pointsPerBin);
  const endIndex = Math.min(Math.floor((index + 1) * pointsPerBin), bufferLength);
  return getAverageFrequency(dataArray, startIndex, endIndex);
} 